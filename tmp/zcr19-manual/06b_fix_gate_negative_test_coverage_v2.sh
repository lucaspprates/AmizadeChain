...
cleanup() {
  ...
  FAILED_FINAL="$OPS/ETAPA-6B-FAILED-$STAMP"
  if sudo -n true >/dev/null 2>&1 && sudo -n test -d "$OPS"; then
    sudo -n rm -rf "$FAILED_FINAL"
    sudo -n install -d -m 0700 -o root -g root "$FAILED_FINAL"
    sudo -n cp -a "$TMP/." "$FAILED_FINAL/"
    sudo -n chown -R root:root "$FAILED_FINAL"
    sudo -n chmod -R go-rwx "$FAILED_FINAL"
    echo "FAILURE_EVIDENCE=$FAILED_FINAL" >&2
    rm -rf "$TMP"
  else
    echo "FAILURE_EVIDENCE_TMP=$TMP" >&2
  fi
}
trap cleanup EXIT
...
echo
echo 'MANUAL_ETAPA_6B_WRITER: STARTED'
echo "JOB_ID=$JOB_ID"
echo "START_SHA=$START_SHA"
echo "WRITER_PROFILE=$WRITER"
echo 'NOTE=terminal may remain quiet while the remote Writer runs'
