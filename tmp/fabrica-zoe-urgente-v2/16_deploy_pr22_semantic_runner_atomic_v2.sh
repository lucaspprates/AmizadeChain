...
SOURCE_PATH="src/zoe_coder_router/zoe_coder_router.py"
TEST_PATH="tests/test_zcr19_semantic_contract.py"
...
STAGE="$(mktemp -d "/tmp/zcr-pr22-deploy-v2-${STAMP}.XXXXXX")"
LOG="/tmp/zcr-pr22-deploy-v2-${STAMP}.log"
BACKUP_DIR="/var/backups/zoe-coder-router/deploy-pr22-v2-${STAMP}"
...
finish() {
  ...
  if [[ "$rc" -ne 0 && "$DEPLOYED" == true ]]; then
    ...
    echo "Falha posterior à troca atômica; restaurando o runtime anterior."
    ...
    RESTORED_SHA256="$(sha256sum "$RUNTIME" 2>/dev/null | awk '{print $1}')"
    if [[ "$RESTORED_SHA256" == "$OLD_RUNTIME_SHA256" ]]; then
      echo "ROLLBACK_RUNTIME: PASS"
      echo "RESTORED_SHA256=$RESTORED_SHA256"
    else
      echo "ROLLBACK_RUNTIME: FAIL" >&2
      echo "RESTORED_SHA256=$RESTORED_SHA256" >&2
      rc=2
    fi
...
[[ -f "$CONFIG" ]] || fail "config.toml ausente"
...
git -C "$REPO_ROOT" cat-file -e "${HEAD_SHA}^{commit}" ||
  fail "commit da PR #22 não existe localmente"

SOURCE_BLOB_READBACK="$(git -C "$REPO_ROOT" rev-parse "${HEAD_SHA}:${SOURCE_PATH}")"
TEST_BLOB_READBACK="$(git -C "$REPO_ROOT" rev-parse "${HEAD_SHA}:${TEST_PATH}")"
[[ "$SOURCE_BLOB_READBACK" == "$NEW_RUNTIME_BLOB" ]] ||
  fail "blob do runtime no commit divergiu: $SOURCE_BLOB_READBACK"
[[ "$TEST_BLOB_READBACK" == "$TEST_BLOB" ]] ||
  fail "blob do teste no commit divergiu: $TEST_BLOB_READBACK"
...
echo
echo "===== 2. STAGING PELO OBJETO GIT EXATO ====="

mkdir -p "$STAGE/canary/src/zoe_coder_router" "$STAGE/canary/tests"

git -C "$REPO_ROOT" show "${HEAD_SHA}:${SOURCE_PATH}" > "$STAGE/zoe_coder_router.py"
git -C "$REPO_ROOT" show "${HEAD_SHA}:${TEST_PATH}" > "$STAGE/test_zcr19_semantic_contract.py"

[[ "$(git hash-object "$STAGE/zoe_coder_router.py")" == "$NEW_RUNTIME_BLOB" ]] ||
  fail "runtime extraído divergiu do blob aprovado"
[[ "$(git hash-object "$STAGE/test_zcr19_semantic_contract.py")" == "$TEST_BLOB" ]] ||
  fail "teste extraído divergiu do blob aprovado"
...
grep -qx 'import re' "$STAGE/zoe_coder_router.py" ||
  fail "import re ausente no candidato"
...
cp "$STAGE/zoe_coder_router.py" \
  "$STAGE/canary/src/zoe_coder_router/zoe_coder_router.py"
...
sudo cp -a "$CONFIG" "$BACKUP_DIR/config.toml.before"
sudo install -m 0600 -o root -g root \
  "$GATE_EVIDENCE/result.json" "$BACKUP_DIR/gate-result.json"

cat > "$STAGE/deployment-admission.json" <<EOF
{
  "deployment": "PR22_SEMANTIC_RUNNER_V2",
  "pr": 22,
  "base_sha": "$BASE_SHA",
  "head_sha": "$HEAD_SHA",
  "source": "local_git_object",
...
sudo install -m 0600 -o root -g root \
  "$STAGE/predeploy-focused-tests.txt" "$BACKUP_DIR/predeploy-focused-tests.txt"
...
sudo bash -c '
  ...
  sha256sum \
    ...
    predeploy-focused-tests.txt > SHA256SUMS.before
' _ "$BACKUP_DIR"
...
sudo install -m "$OLD_MODE" -o "$OLD_UID" -g "$OLD_GID" \
...
[[ "$(stat -c '%u:%g' "$RUNTIME")" == "$OLD_UID:$OLD_GID" ]] ||
  fail "owner do runtime instalado divergiu"
[[ "$(stat -c '%a' "$RUNTIME")" == "$OLD_MODE" ]] ||
  fail "modo do runtime instalado divergiu"
...
echo
echo "PR22_SEMANTIC_RUNNER_DEPLOY_V2: PASS"
echo "PR=22"
