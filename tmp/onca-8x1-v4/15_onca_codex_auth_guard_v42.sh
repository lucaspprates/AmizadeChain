...
EVIDENCE="/tmp/evidence/onca-codex-auth-guard-v42-${STAMP}"
mkdir -p "$EVIDENCE"
...
fail() {
  ...
  shift
  echo "ERRO: $*" >&2
  echo "ONCA_CODEX_AUTH_GUARD_V42: BLOCKED"
...
set +e
"${SSH[@]}" \
  "sudo -n -u '$RUN_USER' -H env -u OPENAI_API_KEY -u OPENAI_BASE_URL -u OPENAI_API_BASE -u CODEX_HOME -u XDG_CONFIG_HOME HOME=/home/$RUN_USER bash -s" \
  <<'REMOTE' 2>&1 | tee "$EVIDENCE/worker-auth-guard.log"
set -Eeuo pipefail
umask 0077
cd /
...
codex exec \
  ...
  </dev/null >"$TMP/inference.jsonl" 2>&1
CODEX_RC=$?
set -e
cat "$TMP/inference.jsonl"
echo "codex_inference_rc=$CODEX_RC"

set +e
python3 - "$TMP/inference.jsonl" <<'PY_AUTH'
import json
import sys
from pathlib import Path
...
if not found:
    raise SystemExit(1)
print("ONCA_CODEX_AUTH_INFERENCE=PASS")
PY_AUTH
AUTH_PARSE_RC=$?
set -e

if [[ "$CODEX_RC" -eq 0 && "$AUTH_PARSE_RC" -eq 0 ]]; then
  echo "AUTH_FAILURE_CODE=NONE"
  echo "WORKER_AUTH_GUARD=PASS"
  exit 0
fi

if grep -Fq 'Missing bearer or basic authentication' "$TMP/inference.jsonl"; then
  FAILURE="MISSING_AUTHORIZATION_HEADER"
elif grep -Fq 'token_invalidated' "$TMP/inference.jsonl"; then
  FAILURE="TOKEN_INVALIDATED"
elif grep -Fq '401 Unauthorized' "$TMP/inference.jsonl"; then
  FAILURE="HTTP_401_AUTHENTICATION"
elif grep -Eq '(^|[^0-9])429([^0-9]|$)|rate.?limit|usage limit' "$TMP/inference.jsonl"; then
  FAILURE="RATE_OR_USAGE_LIMIT"
elif grep -Eq '(^|[^0-9])(500|502|503|504)([^0-9]|$)|service unavailable|overloaded' "$TMP/inference.jsonl"; then
  FAILURE="OPENAI_TRANSIENT_SERVER_ERROR"
elif grep -Eqi 'could not resolve|connection refused|timed out|network is unreachable|TLS|certificate' "$TMP/inference.jsonl"; then
  FAILURE="NETWORK_OR_TLS_FAILURE"
else
  FAILURE="CODEX_INFERENCE_UNCLASSIFIED"
fi

echo "ONCA_CODEX_AUTH_INFERENCE=BLOCKED"
echo "AUTH_FAILURE_CODE=$FAILURE"
...
if [[ "$WORKER_RC" -ne 0 ]]; then
  ...
  fail "$FAILURE_CODE" "a inferência real do Codex não autenticou"
fi
...
echo
cat <<EOF
ONCA_CODEX_AUTH_GUARD_V42: PASS
...
NEXT_PHASE=PR19_RESTACK_GATE_V42
EOF
