...
TARGET="/tmp/11_pr17_exact_sha_gate_v385_patched.sh"
STAMP="$(date -u +%Y%m%dT%H%M%SZ)"
EVIDENCE="/tmp/evidence/zcr17-v385-auth-stdin-${STAMP}"
mkdir -p "$EVIDENCE"

fail() {
  echo "ERRO: $*" >&2
  echo "OPERACAO_ZCR17_V385_WRAPPER: BLOCKED"
...
new_auth = '''  </dev/null 2>&1 | tee "$TMP/auth.jsonl"
...
PY_AUTH
echo WORKER_AUTH_INFERENCE=PASS'''
patches.append(("auth_stream_and_stdin", old_auth, new_auth))
...
grep -Fq '</dev/null 2>&1 | tee "$TMP/auth.jsonl"' "$TARGET" ||
  fail AUTH_STDIN_ISOLATION_MISSING
...
printf '%s  %s\n' "$PATCHED_SHA256" 11_pr17_exact_sha_gate_v385_patched.sh >> "$EVIDENCE/MANIFEST"

cat <<EOF
OPERACAO_ZCR17_V385_WRAPPER: PASS
